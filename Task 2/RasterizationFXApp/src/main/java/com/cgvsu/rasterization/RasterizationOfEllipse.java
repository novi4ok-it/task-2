...
public class RasterizationOfEllipse implements EllipseDrawingService {
    public void drawFilledEllipseByCorners(Canvas canvas, double x, double y, double width, double height, Color startColor, Color endColor) {
    ...
    public static boolean isPointInEllipse(double centerX, double centerY, double a, double b, int x, int y) {
        double dx = x - centerX;
        double dy = y - centerY;
        return (dx * dx) / (a * a) + (dy * dy) / (b * b) <= 1;
    }
    public static Color getGradientColor(Color startColor, Color endColor, double centerX, double centerY, double a, double b, int x, int y) {
